...
| **Tender BOM** | Bóc BOM khối lượng vách/trần, Auto-tracing biên dạng, Link-line và gom Group CAD block | 🟢 Stable (v0.2.6) |
...
2. Chạy script release trong root repo:
   ```powershell
   powershell -ExecutionPolicy Bypass -File .\scripts\release.ps1 -Version 0.2.6
   ```
3. Nếu local đã commit nhưng chưa push:
   ```powershell
   powershell -ExecutionPolicy Bypass -File .\scripts\release.ps1 -Version 0.2.6 -PushLatestCommit
   ```
4. Theo dõi workflow:
   ```powershell
   powershell -ExecutionPolicy Bypass -File .\scripts\watch-release.ps1 -Version 0.2.6
   ```
5. Sau khi workflow xanh, gửi file `ShopDrawing.Setup.0.2.6.zip` cho người cần cài mới.
...
- Baseline deploy on dinh moi nhat: `v0.2.6`.
