...
#pragma warning disable CS8602
        private static void MigrateLegacyAccessory(TenderAccessory accessory)
        {
            ...
            if (string.Equals(application, AppExterior, StringComparison.OrdinalIgnoreCase))
            {
                ...
                // Khôi phục dữ liệu đã từng bị map nhầm: Sealant MS-617 @ khe đứng Omega
                // cần quay về đúng vật tư băng keo butyl Omega.
                if (string.Equals(accessory.Name, SealantMS617, StringComparison.OrdinalIgnoreCase)
                    && string.Equals(accessory.Position, PosKheDung, StringComparison.OrdinalIgnoreCase)
                    && accessory.CalcRule == AccessoryCalcRule.PER_VERTICAL_JOINT_HEIGHT)
                {
                    accessory.Name = "Băng keo butyl Omega";
                }
            }
...
#pragma warning restore CS8602
        ...
        private static bool IsObsoleteConfiguredAccessory(TenderAccessory accessory)
        {
            ...
            if (string.Equals(application, AppExterior, StringComparison.OrdinalIgnoreCase)
                && string.Equals(name, "Bộ cửa đi", StringComparison.OrdinalIgnoreCase))
            {
                return true;
            }

            if (string.Equals(application, AppExterior, StringComparison.OrdinalIgnoreCase)
                && string.Equals(name, "Băng keo butyl", StringComparison.OrdinalIgnoreCase))
            {
                return true;
            }
            ...
            if (string.Equals(application, AppExterior, StringComparison.OrdinalIgnoreCase)
                && (string.Equals(name, "Diềm 05", StringComparison.OrdinalIgnoreCase)
                    || (string.Equals(name, "B2S-TEK 15-15×20 HWFS", StringComparison.OrdinalIgnoreCase)
                        && string.Equals(position, PosMoiNoi, StringComparison.OrdinalIgnoreCase)
                        && accessory.CalcRule == AccessoryCalcRule.PER_JOINT_LENGTH)))
            {
                return true;
            }
        ...
        private static List<TenderAccessory> GetCleanroomDefaults() => new()
        {
            ...
            new() { Name = "Thanh V nhôm (Góc ngoài)", Material = "Nhôm", Position = PosGocNgoai, Unit = "md", CalcRule = AccessoryCalcRule.PER_OUTSIDE_CORNER_HEIGHT, Factor = 1.0, CategoryScope = DefaultCategoryScope, Application = AppCleanroom, Note = "Thanh V nhôm ốp góc ngoài phòng sạch" },
            new() { Name = "Thanh V nhôm (Góc trong)", Material = "Nhôm", Position = PosGocTrong, Unit = "md", CalcRule = AccessoryCalcRule.PER_INSIDE_CORNER_HEIGHT, Factor = 1.0, CategoryScope = DefaultCategoryScope, Application = AppCleanroom, Note = "Thanh V nhôm ốp góc trong phòng sạch" },
            new() { Name = SiliconeSN505, Position = PosGocNgoai, Unit = "chai", CalcRule = AccessoryCalcRule.PER_OUTSIDE_CORNER_HEIGHT, Factor = SiliconeSn505JointBottleFactor * 2.0, CategoryScope = DefaultCategoryScope, Application = AppCleanroom, Note = "Bắn silicone góc ngoài (2 mặt)" },
            new() { Name = SiliconeSN505, Position = PosGocTrong, Unit = "chai", CalcRule = AccessoryCalcRule.PER_INSIDE_CORNER_HEIGHT, Factor = SiliconeSn505JointBottleFactor * 2.0, CategoryScope = DefaultCategoryScope, Application = AppCleanroom, Note = "Bắn silicone góc trong (2 mặt)" },
        ...
        private static List<TenderAccessory> GetColdStorageDefaults() => new()
        {
            ...
            new() { Name = "V bo R40", Material = "Nhôm", Position = PosGocNgoai, Unit = "md", CalcRule = AccessoryCalcRule.PER_OUTSIDE_CORNER_HEIGHT, Factor = 2.0, CategoryScope = DefaultCategoryScope, Application = AppColdStorage, Note = "V bo góc ngoài kho lạnh (2 cạnh)" },
            new() { Name = "V bo R40", Material = "Nhôm", Position = PosGocTrong, Unit = "md", CalcRule = AccessoryCalcRule.PER_INSIDE_CORNER_HEIGHT, Factor = 2.0, CategoryScope = DefaultCategoryScope, Application = AppColdStorage, Note = "V bo góc trong kho lạnh (2 cạnh)" },
            new() { Name = SiliconeSN505, Position = PosGocNgoai, Unit = "chai", CalcRule = AccessoryCalcRule.PER_OUTSIDE_CORNER_HEIGHT, Factor = SiliconeSn505JointBottleFactor * 2.0, CategoryScope = DefaultCategoryScope, Application = AppColdStorage, Note = "Bắn silicone góc ngoài (2 mặt)" },
            new() { Name = SiliconeSN505, Position = PosGocTrong, Unit = "chai", CalcRule = AccessoryCalcRule.PER_INSIDE_CORNER_HEIGHT, Factor = SiliconeSn505JointBottleFactor * 2.0, CategoryScope = DefaultCategoryScope, Application = AppColdStorage, Note = "Bắn silicone góc trong (2 mặt)" }
        };
    }
}
