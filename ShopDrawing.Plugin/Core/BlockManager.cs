...

{
...

    {
        ...
        private const string MushroomBoltMarkerBlockName = "SD_MUSHROOM_BOLT_MARK";
        ...
        public List<ObjectId> DrawAllPanels(List<Panel> panels, Transaction tr)

        {
...

            {
                ...
                var result = DrawPanel(panel, tr, ms);

                if (!result.HatchId.IsNull) allHatchIds.Add(result.HatchId);
                allCreatedIds.AddRange(result.AllIds);
        ...
        public List<ObjectId> DrawCeilingHardware(
            ...
            Transaction tr)
        {
            ...
            CeilingSuspensionLayoutData? layout = CeilingSuspensionPreviewService.BuildLayout(
                boundary,
                suspensionDirection,
                divideFromMaxSide,
                tSpacingMm,
            ...
            foreach (double tPosition in layout.TPositions)
            {
                foreach (var segment in CeilingSuspensionPreviewService.GetScanSegments(boundary, tPosition, layout.RunAlongX))
                {
                    var line = layout.RunAlongX
                        ? new Line(new Point3d(segment.Start, tPosition, 0), new Point3d(segment.End, tPosition, 0))
                        : new Line(new Point3d(tPosition, segment.Start, 0), new Point3d(tPosition, segment.End, 0));

                    line.Layer = "SD_CEILING_T";
                    result.Add(ms.AppendEntity(line));
                    tr.AddNewlyCreatedDBObject(line, true);
                }
            }

            foreach (double mushroomPosition in layout.MushroomPositions)
            {
                foreach (var segment in CeilingSuspensionPreviewService.GetScanSegments(boundary, mushroomPosition, layout.RunAlongX))
                {
                    var line = layout.RunAlongX
                        ? new Line(new Point3d(segment.Start, mushroomPosition, 0), new Point3d(segment.End, mushroomPosition, 0))
                        : new Line(new Point3d(mushroomPosition, segment.Start, 0), new Point3d(mushroomPosition, segment.End, 0));

                    line.Layer = "SD_CEILING_MUSHROOM";
                    result.Add(ms.AppendEntity(line));
                    tr.AddNewlyCreatedDBObject(line, true);
                }
                ...
                foreach (var panel in panels)
                {
                    if (!TryGetMushroomBoltCenter(panel, mushroomPosition, layout.RunAlongX, out Point3d center))
                    {
                        continue;
                    }

                    result.Add(InsertMushroomBoltMarker(db, tr, ms, center));
                }
            }
            return result;
        }

        private (ObjectId HatchId, List<ObjectId> AllIds) DrawPanel(Panel panel, Transaction tr, BlockTableRecord ms)
        {
...

        {
            ...
            tr.AddNewlyCreatedDBObject(pl, true); // panel outline
            ShopdrawingEntityMetadata.ApplyPanelMetadata(ms.Database, tr, pl, panel);
            return pl.ObjectId;
...

        {
            ...
            bool useDistinctReuseHatch = false;
            if (useDistinctReuseHatch && panel.IsReused) // Disabled: reuse is identified by tag/source code, not by a separate hatch style.
            {
        ...
        private static string ResolveDisplayedPanelCode(Panel panel)

        {

            string sourceCode = panel.SourceId?.Trim() ?? string.Empty;

            if (!panel.IsReused || string.IsNullOrWhiteSpace(sourceCode))
...

            {
                ...
                if (sourceCode.EndsWith(suffix, StringComparison.OrdinalIgnoreCase))

                {

                    return sourceCode.Substring(0, sourceCode.Length - suffix.Length);
        ...
        private ObjectId InsertTag(Panel panel, Transaction tr, BlockTableRecord ms)
        {
            ...
            string statusIcon = panel.IsReused ? " \u267b" : (panel.IsCutPanel ? " \u2702" : "");
            string specPart = string.IsNullOrEmpty(panel.Spec) ? "" : $" / {panel.Spec}";

            string tagText = $"{ResolveDisplayedPanelCode(panel)}{statusIcon}{specPart}";
...

            {
...

                {
                    ...
                    bool placeTagOnLeft = panel.IsCutPanel
                        && panel.JointLeft == JointType.Cut
                        && panel.JointRight != JointType.Cut;

                    double outsideX = placeTagOnLeft
                        ? panel.X - textHeight * 2.5
                        : panel.X + drawW + textHeight * 2.5;
                    double midY     = panel.Y + drawH / 2;
        ...
        private static bool TryGetMushroomBoltCenter(
            Panel panel,
            double mushroomPosition,
            bool runAlongX,
            out Point3d center)
        {
            double drawW = panel.IsHorizontal ? panel.LengthMm : panel.WidthMm;
            double drawH = panel.IsHorizontal ? panel.WidthMm : panel.LengthMm;
            double minX = panel.X;
            double maxX = panel.X + drawW;
            double minY = panel.Y;
            double maxY = panel.Y + drawH;
            const double tolerance = 1.0;

            if (runAlongX)
            {
                if (mushroomPosition < minY - tolerance || mushroomPosition > maxY + tolerance)
                {
                    center = default;
                    return false;
                }

                double x = !panel.IsHorizontal
                    ? panel.X + (panel.WidthMm / 2.0)
                    : panel.X + (drawW / 2.0);

                center = new Point3d(x, Math.Min(maxY, Math.Max(minY, mushroomPosition)), 0);
                return true;
            }

            if (mushroomPosition < minX - tolerance || mushroomPosition > maxX + tolerance)
            {
                center = default;
                return false;
            }

            double y = panel.IsHorizontal
                ? panel.Y + (panel.WidthMm / 2.0)
                : panel.Y + (drawH / 2.0);

            center = new Point3d(Math.Min(maxX, Math.Max(minX, mushroomPosition)), y, 0);
            return true;
        }

        private ObjectId InsertMushroomBoltMarker(
            Database db,
            Transaction tr,
            BlockTableRecord ms,
            Point3d position)
        {
            ObjectId markerBlockId = EnsureMushroomBoltMarkerBlock(db, tr);
            double markerScale = Math.Max(40.0, GetDrawingScale(db));

            var blockRef = new BlockReference(position, markerBlockId)
            {
                Layer = "SD_CEILING_BOLT",
                ScaleFactors = new Scale3d(markerScale, markerScale, 1.0)
            };

            ObjectId id = ms.AppendEntity(blockRef);
            tr.AddNewlyCreatedDBObject(blockRef, true);
            return id;
        }

        private ObjectId EnsureMushroomBoltMarkerBlock(Database db, Transaction tr)
        {
            BlockTable bt = (BlockTable)tr.GetObject(db.BlockTableId, OpenMode.ForRead);
            if (bt.Has(MushroomBoltMarkerBlockName))
            {
                return bt[MushroomBoltMarkerBlockName];
            }

            bt.UpgradeOpen();
            var marker = new BlockTableRecord
            {
                Name = MushroomBoltMarkerBlockName,
            ...
            var outerCircle = new Circle(Point3d.Origin, Vector3d.ZAxis, 1.0)
            {
                Layer = "0",
                Color = Color.FromColorIndex(ColorMethod.ByBlock, 0)
            };
            marker.AppendEntity(outerCircle);
            tr.AddNewlyCreatedDBObject(outerCircle, true);

            var horizontal = new Line(new Point3d(-1.3, 0, 0), new Point3d(1.3, 0, 0))
            {
                Layer = "0",
                Color = Color.FromColorIndex(ColorMethod.ByBlock, 0)
            };
            marker.AppendEntity(horizontal);
            tr.AddNewlyCreatedDBObject(horizontal, true);

            var vertical = new Line(new Point3d(0, -1.3, 0), new Point3d(0, 1.3, 0))
            {
                Layer = "0",
                Color = Color.FromColorIndex(ColorMethod.ByBlock, 0)
            };
            marker.AppendEntity(vertical);
...

        {
            ...
            { "SD_CEILING_T",   (150, LineWeight.LineWeight018, true,  "Tuyến T nhôm trần") },
            { "SD_CEILING_MUSHROOM", (2, LineWeight.LineWeight018, true, "Tuyến bulong nấm/thanh C trần") },
            { "SD_CEILING_BOLT",(6,   LineWeight.LineWeight025, true,  "Marker bulong nấm trần") },
            { "SD_CEILING_T_HANGER", (5, LineWeight.LineWeight025, true, "Điểm treo thanh T trần") },
            { "SD_CEILING_MUSHROOM_HANGER", (30, LineWeight.LineWeight025, true, "Điểm treo bulong nấm trần") },
            { "SD_WALL_OUTSIDE_CORNER", (1, LineWeight.LineWeight025, true, "Marker góc ngoài vách trên mặt bằng") },
            { "SD_WALL_INSIDE_CORNER", (3, LineWeight.LineWeight025, true, "Marker góc trong vách trên mặt bằng") },
        };
...

        {
...

            {
                ...
                ShopDrawing.Plugin.Core.PluginLogger.Error("Suppressed exception in BlockManager.cs", ex);
        ...
        public void EntityIdsToGroup(ObjectIdCollection ids, string name, Transaction tr)
        ...
        public List<ObjectId> DrawOpenings(List<Opening> openings, Transaction tr)

        {
...

            {
                ...
                result.AddRange(DrawOpeningBoundary(opening, tr, ms));
        ...
        private List<ObjectId> DrawOpeningBoundary(Opening o, Transaction tr, BlockTableRecord ms)
