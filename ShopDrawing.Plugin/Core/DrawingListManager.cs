...
namespace ShopDrawing.Plugin.Core
{
    ...
    public static class DrawingListManager
    {
        ...
        public static void Sync(Document doc)
        {
            ...
            // Xóa Table cũ + frame/title block cũ của DMBV
            foreach (ObjectId id in psBtr)
            {
                ...
                if (ent is Table)
                {
                    ...
                    || string.Equals(entity.Layer, LayoutManagerEngine.TITLE_LAYER, StringComparison.OrdinalIgnoreCase)

                    || string.Equals(entity.Layer, LayoutManagerEngine.TITLE_FRAME_LAYER, StringComparison.OrdinalIgnoreCase)

                    || string.Equals(entity.Layer, LayoutManagerEngine.TITLE_TEXT_LAYER, StringComparison.OrdinalIgnoreCase)))
                {
                    ent.UpgradeOpen();
                    entity.Erase();
                }
            ...
            catch (System.Exception ex) { ShopDrawing.Plugin.Core.PluginLogger.Warn("Suppressed exception: " + ex.Message);  /* fallback A3 */ }
            ...
            var metrics = LayoutManagerEngine.GetBuiltInSheetMetrics(dmbdPaperSize);

            var drawingListMeta = new LayoutMeta
            ...
            LayoutManagerEngine.DrawBuiltInSheetFrame(psBtr, tr, dmbdPaperSize);

            LayoutManagerEngine.DrawBuiltInTitleBlockForMeta(psBtr, tr, dmbdPaperSize, drawingListMeta);
            ...
            double usableWidth  = metrics.ViewportWidth;
            double usableHeight = metrics.ViewportHeight;
            ...
            double tableX = metrics.ViewportLeft + (usableWidth - tableWidth) / 2.0;
            double viewportTop = metrics.ViewportBottom + metrics.ViewportHeight;

            double tableY = viewportTop - (usableHeight - tableHeight) / 2.0; // Table origin is top-left
        ...
        public static LayoutMeta SaveLayoutMetaPreservingRevision(
...

        {
            ...
            string resolvedProjectName = string.IsNullOrWhiteSpace(projectName)
                ...
                ? existing.ProjectName?.Trim() ?? string.Empty
...

            {
                ...
                GhiChu = revisionContent?.Trim() ?? existing.GhiChu ?? string.Empty
        ...
        private static LayoutMeta ReadLayoutMeta(Database db, Transaction tr, string layoutName)
        {
            ...
            catch (System.Exception ex) { ShopDrawing.Plugin.Core.PluginLogger.Warn("Suppressed exception: " + ex.Message);  return new LayoutMeta(); }
        }
        ...
        private static string? DetectSdPaperSize(Database db, Transaction tr, DBDictionary layoutDict)
        {
            ...
            while (enumerator.MoveNext())
            {
                ...
                catch (System.Exception ex) { ShopDrawing.Plugin.Core.PluginLogger.Warn("Suppressed exception: " + ex.Message);  /* skip unreadable layout */ }
            }
        ...
        private static Table BuildTable(Database db, Transaction tr, List<DrawingEntry> entries)
        {
            ...
            for (int c = 0; c < colCount; c++)
                table.Columns[c].Width = colWidths[c];

            table.Rows[0].Height = 10;
            table.Rows[1].Height = 8;
            table.Rows[2].Height = 8;
            for (int r = 3; r < rowCount; r++)
                table.Rows[r].Height = 7;
