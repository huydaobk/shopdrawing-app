...

{
...

    {
        ...
        public const string VIEW_TITLE_PREFIX = "M?T Ð?NG VÁCH - ";
...

        {
...

            {
                ...
                ShopDrawing.Plugin.Core.PluginLogger.Error("Suppressed exception in LayoutManagerEngine.cs", ex);
...

        {
            ...
            var secondCorner = editor.GetCorner(
                ...
                new PromptCornerOptions("\nCh?n góc 2 c?a vùng layout: ", firstCorner.Value));
...

        {
            ...
            var primaryView = request.CreatePrimaryView();

            var pages = ComputePages(primaryView.Region, scaleDenominator, request.PaperSize, tbH, tbW);
            ...
            return pages
...

                {
                    ...
                    return new LayoutPlanItem(
                        ...
                        LayoutManagerRules.GetViewTitlePrefix(primaryView.Kind) + pageLabel,
                        layoutTabName,
            ...
            string revisionContent)
        {
...

            {
...

                {
                    ...
                    // 2. Offer to pack into an existing compatible layout.
                    if (!packed)
                    {
...

                        {
                            ...
                            var result = System.Windows.MessageBox.Show(
                                ...
                                $"Layout '{possibleLayout}' còn ch? tr?ng.\nB?n có mu?n ghép thêm vào layout này không?\n\n- Yes: Ghép vào layout\n- No: T?o riêng layout m?i\n- Cancel: D?ng l?nh",
                    ...
                    if (packed && !string.IsNullOrWhiteSpace(currentTargetLayout))

                    {

                        actualLayoutName = AddViewportToLayout(doc, currentTargetLayout, insertTopLeft, item.PageExtents, scaleDenominator, item.ViewTitleText, scaleText, revisionContent, item.ViewKind);
...

                    {
                        ...
                        if (vTitle.Contains("(PH?N"))

                        {

                            // If it's a part, remove the part tracking for the layout name and title block base

                            // Wait, the PRD says view title should be W1 (PH?N...). But Tên b?n v?: W1.
                ...
                AppendSecondaryViews(doc, request, scaleDenominator, scaleText, revisionContent, createdLayouts);
        ...
        private void AppendSecondaryViews(
            ...
            List<string> createdLayouts)

        {

            if (request.SecondaryViews == null || request.SecondaryViews.Count == 0)
            ...
            var resolvedConfig = EnsureStripDetected(request.TitleBlockConfig, db);

            var (tbH, tbW) = GetStripReduction(resolvedConfig);

            var geometry = BuildPaperGeometry(request.PaperSize, tbH, tbW);



            foreach (var view in request.SecondaryViews)

            {

                string viewTitleText = LayoutManagerRules.BuildViewTitle(view.Kind, view.UserTitle, 1, 1);

                double reqW = (view.Region.MaxPoint.X - view.Region.MinPoint.X) / scaleDenominator;

                double reqH = (view.Region.MaxPoint.Y - view.Region.MinPoint.Y) / scaleDenominator;
                ...
                foreach (string candidateLayout in createdLayouts)

                {

                    if (!TryFindSpaceInExistingLayout(db, candidateLayout, request.PaperSize, reqW, reqH, out insertTopLeft))

                        continue;



                    targetLayoutName = candidateLayout;
...

                {
                    ...
                    if (!string.IsNullOrWhiteSpace(possibleLayout)

                        && !createdLayouts.Contains(possibleLayout, StringComparer.OrdinalIgnoreCase))

                    {

                        var result = System.Windows.MessageBox.Show(

                            $"Layout '{possibleLayout}' còn ch? tr?ng cho '{viewTitleText}'.\nB?n có mu?n ghép thêm vào layout này không?\n\n- Yes: Ghép vào layout\n- No: T?o layout riêng\n- Cancel: B? qua view này",
...

                        {
                            ...
                            targetLayoutName = possibleLayout;
...

                {
                    ...
                    string actualLayoutName = AddViewportToLayout(
                ...
                string layoutName = $"SD-{DrawingListManager.PeekNextDrawingNumber(doc):D3}";
        ...
        private bool TryFindSpaceInExistingLayout(
...

        {
            ...
            if (!TryResolvePaperSize(layout, out string? paperSize)

                || !string.Equals(paperSize, expectedPaperSize, StringComparison.OrdinalIgnoreCase))
            ...
            if (!TryReadBuiltInUsableBounds(tr, layout, out var usableBounds))
        ...
        private string FindAnyLayoutWithSpace(Database db, PaperGeometry geometry, double reqW, double reqH, out Point2d newTopLeft)
        {
...

            {
                ...
                if (TryFindSpaceInExistingLayout(

                    db,

                    layoutName,

                    ResolvePaperSizeFromGeometry(geometry),
        ...
        private static string ResolvePaperSizeFromGeometry(PaperGeometry geometry)

        {

            string? matched = TryResolvePaperSize(geometry.PaperWidth, geometry.PaperHeight);
        ...
        private static bool TryResolvePaperSize(Layout layout, out string? paperSize)

        {
...

            {
                ...
                paperSize = TryResolvePaperSize(plotSize.X, plotSize.Y);

                return !string.IsNullOrWhiteSpace(paperSize);
...

            {
                ...
                PluginLogger.Error("Suppressed exception while resolving layout paper size.", ex);
        ...
        private static string? TryResolvePaperSize(double width, double height)
        ...
        private static bool TryReadBuiltInUsableBounds(Transaction tr, Layout layout, out UsableBounds usableBounds)

        {
            ...
            var rectangles = new List<RectBounds>();

            var horizontalLines = new List<HorizontalLineBounds>();
...

            {
                ...
                if (tr.GetObject(id, OpenMode.ForRead) is not Polyline polyline
                    ...
                    || !string.Equals(polyline.Layer, SHEET_FRAME_LAYER, StringComparison.OrdinalIgnoreCase))
                ...
                if (TryReadAxisAlignedRectangle(polyline, out var rect))
                ...
                if (TryReadHorizontalLine(polyline, out var line))
            ...
            var inner = rectangles

                .Where(r =>

                    r.Area < outer.Area

                    && Math.Abs(r.MinX - outer.MinX) <= tolerance

                    && Math.Abs(r.MaxX - outer.MaxX) <= tolerance

                    && Math.Abs(r.MaxY - outer.MaxY) <= tolerance

                    && r.MinY > outer.MinY + tolerance)
            ...
            double? viewportBottom = inner?.MinY;

            if (viewportBottom == null)

            {

                viewportBottom = horizontalLines

                    .Where(line =>

                        Math.Abs(line.MinX - outer.MinX) <= tolerance

                        && Math.Abs(line.MaxX - outer.MaxX) <= tolerance

                        && line.Y > outer.MinY + tolerance

                        && line.Y < outer.MaxY - tolerance)
        ...
        private static bool TryReadAxisAlignedRectangle(Polyline polyline, out RectBounds rect)

        {
            ...
            bool isRectangle = xs.All(x => Math.Abs(x - minX) <= tolerance || Math.Abs(x - maxX) <= tolerance)

                && ys.All(y => Math.Abs(y - minY) <= tolerance || Math.Abs(y - maxY) <= tolerance)
            ...
            rect = new RectBounds(minX, maxX, minY, maxY);
        ...
        private static bool TryReadHorizontalLine(Polyline polyline, out HorizontalLineBounds line)
        ...
        private string AddViewportToLayout(
            ...
            string viewTitleText, // The specific title for this vp, e.g. "W2 (PH?N...)"

            string scaleText,

            string revisionContent,

            LayoutViewKind viewKind)

        {
...

            {
                ...
                try { viewport.On = true; } catch (System.Exception ex)

            {
                ...
                if (viewKind == LayoutViewKind.Elevation)

                {

                // --- Naming Logic ---
                // Merge existing layout name (e.g. "M?T Ð?NG VÁCH - W1") with new view title (e.g. "M?T Ð?NG VÁCH - W3")
                ...
                // Update the drawing title, but preserve the current revision metadata.
                DrawingListManager.SaveLayoutMetaPreservingRevision(
                    doc,
...

        {
...

            {
                ...
                // View title n?m ngay du?i viewport th?c t?
...

        {
...

            {
                ...
                // Phase 5: Try Managed .NET API first for maximum version compatibility (AutoCAD 2024+)
...

                {
                    ...
                    // Fallback to P/Invoke if CVPORT mechanism fails or is blocked

                    NativeMethods.TrySetCurrentVPortNative(viewport);
...

            {
                ...
                PluginLogger.Error("Suppressed exception in TryActivateViewport (LayoutManagerEngine.cs).", ex);
...

        {
            ...
            var projectProfile = new ProjectProfileManager().LoadOrDefault();

            var titleValues = new LayoutTitleBlockValueSet

            {

                ProjectName = string.IsNullOrWhiteSpace(meta.ProjectName)
                ...
                ProjectAddress = projectProfile.ProjectAddress,
...

        {
            ...
            // Gi?i h?n viewport không vu?t quá vùng usable
            ...
            // Can viewport v? góc trên-trái vùng usable (chuyên nghi?p hon can gi?a)
...

        {
            ...
            AddText(paperSpace, transaction, "S?A Ð?I", x0 + 1, y0 + height - 6, 2.5, false);
            ...
            AddText(paperSpace, transaction, "N?I DUNG", column2 + 1, y0 + height - 13, 2.0, false);
            ...
            AddText(paperSpace, transaction, "TÊN D? ÁN:", nameBlockX + 2, y0 + height - 5, 2.0, false);
            ...
            AddText(paperSpace, transaction, "TÊN B?N V?:", nameBlockX + 2, y0 + height - 23, 2.0, false);
            ...
            AddText(paperSpace, transaction, "S? B?N V?", nameBlockX + 2, y0 + height - 42, 2.0, false);

            AddText(paperSpace, transaction, "T? L?", nameBlockX + cellWidth + 2, y0 + height - 42, 2.0, false);
        ...
        internal static (double ViewportLeft, double ViewportBottom, double ViewportWidth, double ViewportHeight, double PaperWidth, double PaperHeight)

            GetBuiltInSheetMetrics(string paperSize)

        {

            var engine = new LayoutManagerEngine();

            var geometry = engine.BuildPaperGeometry(paperSize, TITLE_BLOCK_H, 0);
        ...
        internal static void DrawBuiltInSheetFrame(BlockTableRecord paperSpace, Transaction transaction, string paperSize)

        {
            ...
            engine.DrawSheetFrame(paperSpace, transaction, geometry);

        }



        internal static void DrawBuiltInTitleBlockForMeta(
        ...
        /// Removes the page-split suffix like " (PH?N 1/2)" or " (PART 1/2)"
...

        {
            ...
            int phanIdx = title.IndexOf(" (PH?N", StringComparison.OrdinalIgnoreCase);
...

        {
            ...
            // Tìm canonical media name phù h?p t? danh sách plotter h? tr?
...

            {
                ...
                // Fallback: th? set b?ng tên ph? bi?n nh?t
...

                {
                    ...
                    // Gi? nguyên media m?c d?nh c?a plotter
        ...
        /// Duy?t danh sách canonical media names c?a plotter, 

        /// tìm kh? gi?y phù h?p (uu tiên "full bleed", fallback "ISO expand").
...

        {
...

            {
                ...
                // Uu tiên "full bleed" (không margin) ? chính xác nh?t cho shopdrawing
                ...
                // Fallback: b?t k? entry nào ch?a tên kh? gi?y
        ...
        private sealed record RectBounds(double MinX, double MaxX, double MinY, double MaxY)
        ...
        private sealed record HorizontalLineBounds(double MinX, double MaxX, double Y);



        private sealed record UsableBounds(double MinX, double MaxX, double MinY, double MaxY);
        ...
        public sealed record LayoutPlanItem(Extents3d PageExtents, string ViewTitleText, string LayoutName, LayoutViewKind ViewKind);
    }
