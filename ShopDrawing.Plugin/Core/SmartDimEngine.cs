...
namespace ShopDrawing.Plugin.Core
{
    ...
    public class SmartDimEngine
    {
        ...
        public static ObjectId EnsureDimStyle(Database db, Transaction tr)
        {
            ...
            catch (System.Exception ex) { ShopDrawing.Plugin.Core.PluginLogger.Warn("Suppressed exception: " + ex.Message);  /* Older API versions may not support DIMFXL — safe to ignore */ }
        ...
        private static double GetScaleFactor(Database? db = null)
        {
            ...
            {
                ShopDrawing.Plugin.Core.PluginLogger.Error("Suppressed exception in SmartDimEngine.cs", ex);
            }
        ...
        public AutoDimResult AutoDimWall(AutoDimRequest req)
        {
            ...
            try
            {
                ...
                // Detect panel install orientation, not just the overall wall aspect ratio.
                ...
                bool isHorizontal = DeterminePanelSplitAlongX(panels, wallBounds);
            ...
            bool isHorizontal, double dimGap, string wallCode)
        {
            ...
            if (isHorizontal)
            {
                ...
                var orderedPanels = panels
                    ...
                    .OrderBy(p => p.bounds.MinPoint.X)

                    .ThenBy(p => p.bounds.MinPoint.Y)

                    .ToList();

                var distinctWidthPanels = orderedPanels

                    .Where((panel, index) => orderedPanels

                        .Take(index)

                        .All(prev => !AreEquivalentHorizontalSpans(prev.bounds, panel.bounds)))
                ...
                foreach (var p in distinctWidthPanels)
                {
                ...
                var first = distinctWidthPanels.First();
                var last = distinctWidthPanels.Last();
            ...
            bool isHorizontal, double dimGap, bool onRight, int outerLaneOffset, string wallCode)
        {
            ...
            if (isHorizontal)
            {
                ...
                double anchorX = onRight ? wallBounds.MaxPoint.X : wallBounds.MinPoint.X;

                pt1 = new Point3d(anchorX, wallBounds.MinPoint.Y, 0);
                pt2 = new Point3d(anchorX, wallBounds.MaxPoint.Y, 0);
            ...
            double dimGap, string wallCode)
        {
...

            {
                ...
                elevations.Add(opening.Bounds.MinPoint.Y);

                elevations.Add(opening.Bounds.MaxPoint.Y);
        ...
        public bool ManualDim(double? rotationRad = null)
        {
            ...
            catch (System.Exception ex) { ShopDrawing.Plugin.Core.PluginLogger.Warn("Suppressed exception: " + ex.Message);  tr.Abort(); return false; }
        }
            ...
            BlockTableRecord ms, Transaction tr, string wallCode)
        {
            ...
            foreach (ObjectId id in ms)
            {
                ...
                bool isHorizontalInstall = IsHorizontalInstallTag(txt);
        ...
        private static bool IsHorizontalInstallTag(DBText txt)

        {

            double normalized = txt.Rotation % Math.PI;
        ...
        private static bool TryFindPanelForTag(
            ...
            Point3d tagPoint,
            double textH,
            [NotNullWhen(true)] out PanelPos? panel)
        {
            PanelPos? bestPanel = null;
            ...
            foreach (ObjectId id in ms)
            {
                ...
                if (bestPanel == null || score < bestScore)
                {
                    bestScore = score;
                    bestPanel = candidate;
                }
        ...
        private static bool TryBuildPanelPos(Polyline pl, double textH, [NotNullWhen(true)] out PanelPos? panel)
        {
            try
            {
                var ext = pl.GeometricExtents;
                ...
                panel = new PanelPos(start, Math.Max(w, h), ext, ext, Point3d.Origin, textH, w >= h);
                return true;
            }
            catch (System.Exception ex) { ShopDrawing.Plugin.Core.PluginLogger.Warn("Suppressed exception: " + ex.Message); 
                panel = null;
        ...
        private static bool DeterminePanelInstallOrientation(List<PanelPos> panels, Extents3d wallBounds)

        {

            int horizontalCount = panels.Count(p => p.isHorizontalInstall);

            int verticalCount = panels.Count - horizontalCount;
            ...
            double wallWidth = wallBounds.MaxPoint.X - wallBounds.MinPoint.X;

            double wallHeight = wallBounds.MaxPoint.Y - wallBounds.MinPoint.Y;
        ...
        private static bool DeterminePanelSplitAlongX(List<PanelPos> panels, Extents3d wallBounds)

        {

            int distinctXSpans = CountDistinctPanelSpans(panels, alongX: true);

            int distinctYSpans = CountDistinctPanelSpans(panels, alongX: false);

            if (distinctXSpans != distinctYSpans)

            {

                return distinctXSpans > distinctYSpans;

            }



            // Fallback: if span counts are tied (e.g. only one panel),
            // use panel install direction to infer module split axis.
            // Horizontal install => modules split along Y (not X).
            // Vertical install   => modules split along X.
            return !DeterminePanelInstallOrientation(panels, wallBounds);

        }



        private static int CountDistinctPanelSpans(List<PanelPos> panels, bool alongX, double tolerance = 1.0)

        {

            var distinct = new List<Extents3d>();

            foreach (var panel in panels)

            {

                bool exists = distinct.Any(existing => alongX

                    ? AreEquivalentHorizontalSpans(existing, panel.bounds, tolerance)

                    : AreEquivalentVerticalSpans(existing, panel.bounds, tolerance));

                if (!exists)

                {

                    distinct.Add(panel.bounds);
        ...
        private static bool AreEquivalentHorizontalSpans(Extents3d left, Extents3d right, double tolerance = 1.0)

        {

            return Math.Abs(left.MinPoint.X - right.MinPoint.X) <= tolerance &&

                   Math.Abs(left.MaxPoint.X - right.MaxPoint.X) <= tolerance;

        }



        private static bool AreEquivalentVerticalSpans(Extents3d left, Extents3d right, double tolerance = 1.0)

        {

            return Math.Abs(left.MinPoint.Y - right.MinPoint.Y) <= tolerance &&

                   Math.Abs(left.MaxPoint.Y - right.MaxPoint.Y) <= tolerance;
