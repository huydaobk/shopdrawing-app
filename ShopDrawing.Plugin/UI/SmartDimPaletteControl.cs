...
{
    /// <summary>
    /// Smart Dim Palette — dark theme, unified with other palettes.
    /// </summary>
    public class SmartDimPaletteControl : UserControl
    {
        ...
        private readonly CheckBox    _chkWidth, _chkHeight, _chkOpening, _chkElevation;
        ...
        public SmartDimPaletteControl()
        {
            ...
            var lblTuong = SdPaletteStyles.CreateLabel("Tường:");
            lblTuong.VerticalAlignment = VerticalAlignment.Center;
            lblTuong.Margin = new Thickness(0, 0, 4, 0);
            Grid.SetColumn(lblTuong, 0);
            row1.Children.Add(lblTuong);

            _cboWall = new ComboBox
            {
                IsEditable = true, Text = "W1",
                ToolTip = "Chọn mã tường hoặc nhập thủ công",
                VerticalContentAlignment = VerticalAlignment.Center, Height = 24
            };
            Grid.SetColumn(_cboWall, 1);
            row1.Children.Add(_cboWall);

            var btnScan = SdPaletteStyles.CreateCompactButton("🔍");
            btnScan.Width = 26; btnScan.Height = 24;
            btnScan.Margin = new Thickness(4, 0, 8, 0);
            btnScan.ToolTip = "Scan bản vẽ → tìm tất cả mã tường";
            btnScan.Click += (s, e) => ScanWallCodes();
            Grid.SetColumn(btnScan, 2);
            row1.Children.Add(btnScan);

            var lblCao = SdPaletteStyles.CreateLabel("Cao:");
            lblCao.VerticalAlignment = VerticalAlignment.Center;
            lblCao.Margin = new Thickness(0, 0, 4, 0);
            Grid.SetColumn(lblCao, 3);
            row1.Children.Add(lblCao);

            _cboHeightSide = new ComboBox
            {
                ToolTip = "Wall ngang: Phai/Trai. Wall dọc: Trên/Dưới.",
                Height = 24, VerticalContentAlignment = VerticalAlignment.Center
            };
            ...
            _chkElevation = MakeCheckBox("Elev", true); _chkElevation.Margin = new Thickness(0);
            ...
            // Auto buttons
            var row3 = new Grid { Margin = new Thickness(0, 0, 0, 0) };
            ...
            Grid.SetColumn(btnCreate, 0); btnCreate.Margin = new Thickness(0, 0, 4, 0);
            btnCreate.Click += (s, e) => RunAutoDim();
            row3.Children.Add(btnCreate);

            var btnDelete = SdPaletteStyles.CreateActionButton("🗑 Xóa Dim", SdPaletteStyles.BtnDefaultBrush);
            Grid.SetColumn(btnDelete, 1); btnDelete.Margin = new Thickness(4, 0, 0, 0);
            btnDelete.Click += (s, e) => DeleteDims();
            ...
            Grid.SetColumn(btnH, 0); btnH.Margin = new Thickness(0, 0, 4, 0);
            btnH.Click += (s, e) => RunManualLoop(0);
            rowManual.Children.Add(btnH);

            var btnV = SdPaletteStyles.CreateOutlineButton("↕ Dim Dọc");
            Grid.SetColumn(btnV, 1); btnV.Margin = new Thickness(4, 0, 0, 0);
            btnV.Click += (s, e) => RunManualLoop(Math.PI / 2);
        ...
        private void ScanWallCodes()
        {
            try
            {
                ...
                foreach (var c in codes) _cboWall.Items.Add(c);
                if (codes.Count > 0)
                {
                    if (codes.Contains(prev)) _cboWall.Text = prev;
                    else _cboWall.SelectedIndex = 0;
                    SetStatus($"🔍 Tìm thấy {codes.Count} tường: {string.Join(", ", codes)}", false);
                }
                else { SetStatus("⚠️ Không tìm thấy tường nào (chưa có SD_TAG).", true); }
            }
            catch (Exception ex) { SetStatus($"❌ Scan lỗi: {ex.Message}", true); }
        }
        ...
        private void RunAutoDim()
        {
            try
            {
                string wallCode = (_cboWall.Text ?? "").Trim().ToUpper();
                if (string.IsNullOrEmpty(wallCode)) { SetStatus("⚠️ Chọn mã tường trước.", true); return; }
        ...
        private void DeleteDims()
        {
            try
            {
                ...
                SetStatus(count > 0 ? $"🗑 Đã xóa {count} dim của {wallCode}." : $"⚠️ Không tìm thấy dim nào cho {wallCode}.", count == 0);
            }
            catch (Exception ex) { SetStatus($"❌ {ex.Message}", true); }
        }
        ...
        private void RunManualLoop(double rotation)
        {
            try
            {
                ...
                SetStatus(count > 0 ? $"✅ Đã vẽ {count} dim thủ công." : "⚠️ Đã huỷ.", count == 0);
            }
            catch (Exception ex) { SetStatus($"❌ {ex.Message}", true); }
        }
        ...
        {
            _lblStatus.Text = msg;
            _lblStatus.Foreground = isWarning ? SdPaletteStyles.AccentOrangeBrush : SdPaletteStyles.AccentGreenBrush;
        }
