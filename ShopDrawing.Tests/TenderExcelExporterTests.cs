...
namespace ShopDrawing.Tests
{
    public class TenderExcelExporterTests
    {
        ...
        [Fact]
        public void Export_ShouldIncludeSpecManagementSheetInSameWorkbook_CurrentContract()
        {
            string tempFile = Path.Combine(Path.GetTempPath(), $"tender-export-{Guid.NewGuid():N}.xlsx");
            ...
            try
            {
                ...
                var specSheet = workbook.GetSheet("Quản lý Spec");
                Assert.NotNull(specSheet);
                Assert.Equal("Mã spec", specSheet.GetRow(5).GetCell(1).StringCellValue);
                Assert.Equal("SP-01", specSheet.GetRow(6).GetCell(1).StringCellValue);
                Assert.Equal(1100, specSheet.GetRow(6).GetCell(3).NumericCellValue);
                Assert.Equal("ISOFRIGO", specSheet.GetRow(6).GetCell(4).StringCellValue);
                Assert.Equal("Có", specSheet.GetRow(6).GetCell(8).StringCellValue);
            }
            finally
            {
                if (File.Exists(tempFile))
