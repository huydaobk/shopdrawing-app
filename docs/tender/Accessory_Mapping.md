...
| Mastic tape | Mối nối | md | `PER_JOINT_LENGTH` | 1.0 | |
| Rivet Ø4.2×12 | Vách | cái | `PER_WALL_LENGTH` | 6.0 | |

### 2. Phòng sạch

| Name | Position | Unit | CalcRule | Factor | Ghi chú |
|---|---|---|---|---:|---|
| Nẹp nhôm phẳng | Mối nối | md | `PER_JOINT_LENGTH` | 1.0 | |
| Nẹp chân vệ sinh | Cạnh dưới | md | `PER_BOTTOM_EDGE_LENGTH` | 1.0 | |
...
| Silicone SN-505 | Mối nối | md | `PER_JOINT_LENGTH` | 1.0 | Kháng mốc |
...
| B2S-TEK inox | Panel→thép | cái | `PER_PANEL_QTY` | 12.0 | |
| Rivet inox Ø4.2×12 | Vách | cái | `PER_WALL_LENGTH` | 6.0 | |
...
- `Thả cáp (mm)` được nhập tay trực tiếp trên bảng Tender để quy đổi `Wire rope Ø12`.
...
| Sealant MC-202 |  | Mối nối | md | `PER_JOINT_LENGTH` | 1.0 | Không thấy callout trong bộ detail PDF này |
...
| Rivet Ø4.2×12 | Nhôm | Vách | cái | `PER_WALL_LENGTH` | 6.0 | Gần đúng cho spacing `@500`, nhưng mới là xấp xỉ |
...
| Foam on site | Foam PU | Mối nối / góc trong | md | `PER_JOINT_LENGTH` | 1.0 | Tham chiếu | Về bản chất nên quy đổi từ thể tích khe; hiện chưa có rule thể tích |
| Rivet nhôm Ø4.2×12 | Nhôm | Tuyến có rivet @500 | cái | `PER_WALL_LENGTH` | 2.0 | Dùng tạm | Spacing 500 mm tương đương 2 cái/m; nếu cần chính xác theo từng line phải có rule đếm theo tuyến detail |
| U-channel 40x153x40 | Thép | Chân vách / nền | md | `PER_BOTTOM_EDGE_LENGTH` | 1.0 | Dùng được ngay | Theo DT-14 |
...
| Apolo foam 750 ml | Foam PU | Chân vách / nền | md | `PER_BOTTOM_EDGE_LENGTH` | 1.0 | Tham chiếu | Nên quy đổi theo thể tích khe, chưa có rule riêng |
...
| T-profile 68x75 | Nhôm | Điểm treo trần | cái | `PER_OPENING_QTY` | 1.0 | Tham chiếu | Anodized; DT-15 là cấu tạo 1 điểm treo; code chưa có dữ liệu số điểm treo nên chưa tự tính đúng |
...
| C-channel 100x50 | Thép mạ kẽm | Điểm treo trần | cái | `PER_OPENING_QTY` | 1.0 | Tham chiếu | Chỉ xuất hiện ở DT-16 |
...
- Phụ kiện nhôm ghi quy cách trong `Name`, còn `Material = Nhôm`, ví dụ `V 80x80`, `Bo góc R40`, `T-profile 68x75`.
...
| Thép hình / bản mã / ty ren / c-channel | Ghi quy cách hoặc tên cấu kiện | `Thép`, `Thép mạ kẽm` | `Name = Bản mã 50x325x3`, `Material = Thép` |
...
| Fastener / rivet / vít / cùm cáp | Ghi tên + quy cách | Để trống hoặc ghi vật liệu nếu cần tách loại | `Name = Rivet Ø4.2×12`, `Name = Vít No.5 + tắc kê nhựa` |
