...
while i < len(lines):
    ...
    if 'public void DrawAllPanels(List<Panel> panels, Transaction tr)' in line:
        ...
        new_lines.append(line)
        if '}' in lines[i+1]:
            new_lines.append('            return allCreatedIds;\n')
        ...
        new_lines[-1] = new_lines[-1].replace(';', ');')
    elif 'private ObjectId DrawPanel(' in line:
        # Before adding this line, we need to add return result to the end of DrawCeilingHardware.
        # Find the last '        }\r\n' or '        }\n' before this line
        idx = len(new_lines) - 1
        while idx >= 0:
            if new_lines[idx].strip() == '}':
                # this is the end of the class or the end of the previous method
                # wait, let's insert right before the last '}'
                break
            idx -= 1
        if idx >= 0:
            new_lines.insert(idx, '            return result;\n')
